...
　　 }
    ...
    jQuery(function($) {
        ...
        $('#fuelux-wizard').ace_wizard().on('change' , function(e, info){
            ...
            var choices = $("#step" + info.step + " select[name^='itemYes']");
            var itemInputs = $("#step" + info.step + " input[name^='itemInput']");
            var isOk = true;
            for (var i=0;i<choices.length;i++) {
                if (choices[i].value=='N'&&trim(itemInputs[i].value)=='') {
                    isOk = false;
                }
            }
            if (!isOk) {
                bootbox.dialog({
                    message: "必须为选'否'的项目添加问题描述",
                    buttons: {
                        "success" : {
                            "label" : "确定",
        	...
        	else if ($("#form-field-select-2").val()==null||$("#form-field-select-2").val()=="") {
        		bootbox.dialog({
                        message: "请至少选择一个重点评估项!",
                        buttons: {
                            "success" : {
